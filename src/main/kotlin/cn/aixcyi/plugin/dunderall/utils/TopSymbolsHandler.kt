...
class TopSymbolsHandler(private val withImports: Boolean = false, private val depth: Int = 5) {
    ...
    private fun collect(statement: PyStatement) {
        ...
        // 赋值表达式
            ...
            for (pair in statement.targetsToValuesMapping) {
                ...
                if (variable is PyTargetExpression) {
                    ...
                    if (qName?.toString() != variableName) {
                        continue
                    }
                }
                // 公开变量（即不以下划线开头命名的）
                ...
                // 除了 __all__ 以外的特殊变量
                    ...
                    symbols[variableName] = AppIcons.Variable
                }
            }
        }
        // 判断语句
        ...
        // from 根包.子包.孙包 import *
            ...
            for (element in statement.importElements) {
                symbols[element.visibleName ?: continue] = AllIcons.Nodes.Include
            }
        }
        // import 根包
        // import 根包.子包
     ...
     */
    private fun parseStarImport(statement: PyFromImportStatement) {
        if (depth <= 0)
            return
        ...
        if (file.dunderAll != null) {
            file.dunderAll!!.forEach {
                symbols[it] = AllIcons.Nodes.Include
            }
        } else {
